...
def getRightDiags(board, player):
    diags = ['' for i in range(15)]
    for i in range(15):
        if board[i][8 - i] == 3 - player:
            diags[0] += 'x'
        elif board[i][12 - i] == 0:
            diags[0] += 'o'
        elif board[i][12 - i] == player:
            diags[0] += '*'
    for i in range(10):
        for j in range(15):
    ...
    for i in range(10):
        for j in range(15):
            if (7 - j) - i < 0 or board[j][(7 - j) - i] == 3 - player:
                diags[i + 5] += 'x'
            elif board[j][(13 - j) - i] == 0:
                diags[i + 5] += 'o'
            elif board[j][(7 - j) - i] == player:
                diags[i + 5] += '*'
    ...
    def minimaxHelper(self, currPlayer, depth, alpha, beta):
        ...
        if block == False:
            for score, row, col in moves:
                ...
                score = -1*self.minimaxHelper(nextPlayer, depth - 1, -beta, -alpha)
